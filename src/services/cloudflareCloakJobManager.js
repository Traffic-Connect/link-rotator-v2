...
} = require('./cloudflareCloakService');
...
function ensureNoActiveJob() {
    if (currentJob.status === JOB_STATUS.RUNNING) {
        const error = new Error('A Cloudflare cloak job is already running');
        error.statusCode = 400;
        throw error;
    }
}

function initializeJob({ action, domains, requestedBy, config = null }) {
...
async function processDomain(action, domainEntry, config) {
    switch (action) {
        case 'create':
            return createCloakForDomain(domainEntry, config);
        case 'delete':
            return removeCloakForDomain(domainEntry);
        default:
